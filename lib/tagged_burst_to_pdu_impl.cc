      ...
      d_num_threads(num_threads)
{
    ...
    GR_LOG_DEBUG(d_logger, str(boost::format("filter len = %1%") % d_taps.size()));
...
{
    GR_LOG_INFO(d_logger, str(boost::format("Stopped with %d bursts remaining in queue") % d_bursts.size()));
    GR_LOG_INFO(d_logger, str(boost::format("Emitted %lu bursts") % ++d_max_id));
    ...
    for (size_t i = 0; i < work_buffer.new_burst_tags.size(); i++) {
        ...
        if (d_lower_border < relative_frequency && relative_frequency <= d_upper_border) {
            ...
            if (d_bursts.count(id)) {
                GR_LOG_ERROR(d_logger,
                             str(boost::format("tagged_burst_to_pdu saw a repeated burst id "
                                           "(%1%), discarding...") % id));
            ...
            if (id == 1) {
                GR_LOG_INFO(d_logger,
                            str(boost::format("start = %1% %2%") % start_offset % tag.offset));
            }
		...
		std::fill_n(burst.rot_tmp, 4, 0);
            ...
            if (burst.data != NULL) {
                ...
                if (d_debug) {
                    GR_LOG_INFO(d_logger,
                                str(boost::format("New burst: offset=%lu, id=%lu, "
...
void tagged_burst_to_pdu_impl::publish_and_remove_old_bursts(const buffer& work_buffer)
{
    for (tag_t tag : work_buffer.gone_burst_tags) {
        ...
        if (d_bursts.count(id)) {
            ...
            if (d_debug) {
                GR_LOG_INFO(d_logger,
                            str(boost::format("gone burst: %1% %2%") % id % burst.len));
            }
            ...
            if (burst.len >= d_min_burst_size) {
                if (id == 1) {
                    GR_LOG_INFO(d_logger,
                                str(boost::format("id %1%: len = %2% %3%") % id % burst.len %
                                    tag.offset));
                }
                publish_burst(burst);
            }
    ...
    for (std::pair<const uint64_t, burst_data>& burstpair : d_bursts) {
        ...
        if (burst.len >= d_max_burst_size) {
            ...
            if (d_debug) {
                GR_LOG_INFO(d_logger,
                            str(boost::format("gone (long) burst: %1% %2%") % id % burst.len));
            }
            if (burst.len >= d_min_burst_size) {
                publish_burst(burst);
            }
    ...
    while (true) {
        if (d_work_queue.pop(work_buffer)) {
            ...
            if (d_bursts.size() > max_cbursts) {
                GR_LOG_INFO(d_logger,
                            str(boost::format("New max bursts from %1% to %2%") %
