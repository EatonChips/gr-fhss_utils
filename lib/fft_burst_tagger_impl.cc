...

{
    ...
    if (d_debug) {
        GR_LOG_INFO(d_logger,
                    str(boost::format("threshold=%f, d_threshold=%f (%f/%d)") % threshold %
    ...
    GR_LOG_INFO(d_logger, str(boost::format("bursts width is +/- %d FFT bins") % (d_burst_width / 2)));

    d_filter_bandwidth = 0;

    if (d_debug) {
        GR_LOG_INFO(d_logger, str(boost::format("d_max_bursts=%d") % d_max_bursts));
    }
...
{
    GR_LOG_INFO(d_logger, str(boost::format("Tagged %lu bursts") % d_n_tagged_bursts));
    delete d_fft;
    delete d_fine_fft;
...
{
    GR_LOG_INFO(d_logger, str(boost::format("total time: %f") % d_total_timer.elapsed()));
...
#ifdef DO_TIMER
    GR_LOG_INFO(d_logger, str(boost::format("total time: %f") % d_total_timer.elapsed()));
    GR_LOG_INFO(d_logger, str(boost::format("rel mag: %f") % d_rel_mag_timer.elapsed()));
    GR_LOG_INFO(d_logger, str(boost::format("fft time: %f") % d_fft_timer.elapsed()));
    GR_LOG_INFO(d_logger,
                str(boost::format("update pb time: %f") % d_update_pb_timer.elapsed()));
    GR_LOG_INFO(d_logger,
                str(boost::format("update ab time: %f") % d_update_ab_timer.elapsed()));
    GR_LOG_INFO(d_logger,
                str(boost::format("remove tb time: %f") % d_remove_tb_timer.elapsed()));
    GR_LOG_INFO(d_logger, str(boost::format("extract time: %f") % d_extract_timer.elapsed()));
    GR_LOG_INFO(d_logger, str(boost::format("delete time: %f") % d_delete_timer.elapsed()));
    GR_LOG_INFO(d_logger, str(boost::format("new pb time: %f") % d_new_pb_timer.elapsed()));
    GR_LOG_INFO(d_logger, str(boost::format("new burst time: %f") % d_new_b_timer.elapsed()));
    GR_LOG_INFO(d_logger, str(boost::format("update cb: %f") % d_update_cb_timer.elapsed()));
    GR_LOG_INFO(d_logger, str(boost::format("other timer: %f") % d_other.elapsed()));
#endif

    GR_LOG_INFO(d_logger, str(boost::format("saw %lu ffts") % d_abs_fft_index));
    GR_LOG_INFO(d_logger, str(boost::format("extra = %1%") % extra));
    ...
    for (size_t i = b.start_bin; i <= b.stop_bin; i++) {
        ...
        if (d_mask_owners[i].push_back(b.id)) {
            GR_LOG_WARN(d_logger,
                        str(boost::format("Owners::push_back - Trying to add too many points "
                                      "for  bin id=%1%, size=%2%, burst=%3%") %
                            d_mask_owners[i].uid % d_mask_owners[i]._size % b.id));
            GR_LOG_WARN(d_logger,
                        str(boost::format("Owner bins are %1%, %2%, %3% %4%") %
    ...
    if (pb.start_bin != b.start_bin || pb.stop_bin != b.stop_bin) {
        ...
        for (int i = pb.start_bin; i <= pb.stop_bin; i++) {
            ...
            if (d_mask_owners[i].erase(pb.id)) {
                GR_LOG_ERROR(
                    d_logger,
                    str(boost::format("Owners::Remove - Couldn't find id to erase. This "
        ...
        for (int i = b.start_bin; i <= b.stop_bin; i++) {
            ...
            if (d_mask_owners[i].push_back(b.id)) {
                GR_LOG_WARN(d_logger,
                            str(boost::format("Owners::push_back - Trying to add too many "
...
void fft_burst_tagger_impl::preload_noise_floor(double noise_density, bool preload)
{
    ...
    if (preload) {
        GR_LOG_INFO(d_logger, str(boost::format("initializing noise denisty to %.2f dB") % noise_density));
